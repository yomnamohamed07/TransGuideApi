    ...
    public class RouteProfile : Profile
    {
        public RouteProfile()
        {
            CreateMap<Route, RouteDto>()
                ...
                .ForMember(dest => dest.RouteDetails, opt => opt.MapFrom(src => new List<RouteDetailDto>
                {
                    new RouteDetailDto
                    {
                        RouteName = src.Name,
                        Stations = src.RouteStations
                                     .OrderBy(rs => rs.Order)
                ...
                .ForMember(dest => dest.RouteLengthInKm, opt => opt.MapFrom(src =>
                    CalculateRouteLength(src.RouteStations.OrderBy(rs => rs.Order)
        ...
        private double CalculateDistance(Station a, Station b)
        {
            ...
            var aCalc = System.Math.Sin(dLat / 2) * System.Math.Sin(dLat / 2) +
